...
def russia(request):
   ...
   # #                                    locals()--打包
        ...
        allrussia = Russia.objects.all().order_by('id') #.order_by排序('id':遞增,'-id':遞減)
        content = {'russia':allrussia} #資料庫名稱相符
        return render(request,'russia.html',content)